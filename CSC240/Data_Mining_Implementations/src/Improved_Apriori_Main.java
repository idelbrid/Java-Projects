...
public class Improved_Apriori_Main {
	...
	public static String [][] toArray(File DB_file) throws FileNotFoundException{
		Scanner file_scanner = new Scanner(new FileReader(DB_file));
		int count = 0;
		...
		String [][] DB = new String[count][2];
		file_scanner.close();
		file_scanner = new Scanner(new FileReader(DB_file));
		int i=0;
		while(file_scanner.hasNextLine()){
			DB[i][0] = "";
			DB[i][1] = file_scanner.nextLine();
			i++;
		}
		file_scanner.close();
		return DB;
